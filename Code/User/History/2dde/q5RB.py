...
def mergeSort(sortList, swap=True):
    if len(sortList) <= 1:
        return sortList  # Return only the sorted list if length is 1 or less
    ...
    if not swap:
        return mergedList  # Return only the sorted list
    else:
        totalSwaps = mergeSwaps + (0 if isinstance(leftHalf, list) else 0)  # Only for swap counting
        return mergedList, totalSwaps  # Return both sorted list and swap count
...
def merge(left, right):
    ...
    while i < len(left) and j < len(right):
        if left[i] <= right[j]:
            ...
            swaps += (len(left) - i)  # Count remaining elements in left
...
def efficiency_test(n):
    ...
    return bubbleSort(n, swap=True), mergeSort(n, swap=True)
